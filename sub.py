...
def on_message(client, userdata, msg):
    ...
    timestamp_recv = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    ...
    point = (
        ...
        .time(timestamp_device, WritePrecision.S)  # gunakan waktu dari device
    )
    write_api.write(bucket=INFLUX_BUCKET, org=INFLUX_ORG, record=point)

# ---- Setup MQTT ----
mqtt_client = mqtt.Client()
mqtt_client.username_pw_set(USERNAME, PASSWORD)   # <<< tambahkan ini
